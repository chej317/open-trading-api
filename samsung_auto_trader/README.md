...
## 7. 실무적 한계 및 교차 검증상의 이슈 (Limitations & Verification Issues)

실제 시스템을 연속 운영하는 과정에서 **로컬 상태 데이터(`trade_state.json`)가 계산한 누적 수익**과 **한국투자증권(KIS) 모의투자 서버 상의 계좌 수익** 간에 불일치가 발생하는 실무적 괴리가 발견되었습니다. 또한, 이를 검증하는 과정에서도 기술적 장애가 수반됩니다.

### 7.1. 로컬-서버 간 수익 불일치 원인 분석
1. **수수료 및 거래 비용 세부 왜곡**:
   - 로컬 상태 엔진은 순수한 매매 가격 편차($\text{Sell Price} - \text{Buy Price}$)만을 기준으로 단순 거래 성과(PnL)를 합산합니다. 
   - 반면, 실제 한투 모의 서버에서는 매매 거래세(0.18%) 및 위탁 수수료가 즉시 차감되어 반영되므로 거래 횟수가 누적될수록 로컬 수익이 서버 수익보다 과다 산정되는 오차가 발생합니다.
2. **이동평균법 기반 평단가(Avg Price) 계산 차이**:
   - 로컬 시스템은 단일 매수 분량과 매도 분량을 1:1 매칭하여 청산(Round-trip) 시의 수익을 확정하는 구조입니다.
   - 하지만 한국투자증권 서버는 동일 종목을 분할 매수하거나 기존 물량이 남아 있을 때 **이동평균법(Moving Average Method)**으로 평균단가를 동적으로 재조정합니다. 이로 인해 부분 매매 청산 시점의 로컬 장부 수익률과 KIS 잔고상의 수익률 간에 왜곡이 유발됩니다.
3. **모의 서버 체결 지연 및 누락 (Slippage)**:
   - 모의 체결 서버의 지연으로 인해 로컬에서는 주문이 취소되거나 완료되었다고 판정한 시점과, 실제 브로커 데이터베이스에 반영되는 시점의 불일치가 발생합니다. 이 지연은 결국 양측의 보유 수량 및 잔고 차이로 누적됩니다.

### 7.2. 교차 검증(Cross-Verification) 상의 기술적 난점
1. **모의투자 전용 조회 API(TR)의 불안정성**:
   - 본 프로그램은 로컬 손익 신뢰성을 보장하기 위해 `verify_kis_*.py` 스크립트를 제공하지만, KIS 모의투자 환경에서는 **기간별 손익 조회 API**나 **일자별 체결 내역 조회 API**가 빈 데이터(Null)를 반환하거나 수시로 에러(`EGW02006`: 처리 권한 없음)를 뱉는 구조적 불안정성이 있습니다.
2. **엄격한 TPS 제한(Rate Limiting)으로 인한 검증 데이터 누락**:
   - 모의투자는 초당 조회 제한(TPS $\le 2$회)이 매우 엄격합니다. 체결 대조 스크립트가 과거의 다량의 체결 내역을 페이징하여 긁어올 때, 쿨타임 부족으로 인한 타임아웃이나 호출 거부가 빈번히 발생해 검증 데이터 자체가 누락되어 정합성 대조가 물리적으로 실패하는 경우가 존재합니다.

### 7.3. 대응 방안
- 정합성 왜곡이 의심될 경우, 검증용 분석 도구의 모의 서버 에러 로그를 참고한 뒤 **`trade_state.json`을 강제 초기화하여 실제 KIS 잔고 상태(보유량 및 평단가)와 동기화**하여 재시작하는 정책을 권장합니다.
