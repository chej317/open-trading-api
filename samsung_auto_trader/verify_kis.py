...
def main():
    ...
    # 기간: config.py에 정의된 리포트 운영기간에 맞춤 (FINAL_PERFORMANCE_REPORT 기준)
    start_date = "20260526"
    end_date = "20260617"
    ...
    print(json.dumps({"info": "calling inquire-period-trade-profit", "params": {k: (v if k not in ['CANO','ACNT_PRDT_CD'] else 'REDACTED') for k,v in params.items()}}, ensure_ascii=False))

    res = client.get("/uapi/domestic-stock/v1/trading/inquire-period-trade-profit", "TTTC8715R", params=params)

    # 출력: 전체 응답(민감정보 제외)와 요약(가능하면 output2의 합계)
    if not res:
